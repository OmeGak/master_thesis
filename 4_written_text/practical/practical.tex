...
\multicolumn{2}{|c|}{\cellcolor{orange} photon tallying} \\ \hline
...
\newpage
\subsection{Geometry}
\label{geometry_scattering}

Spherical symmetry. Scattering at point $P$
\begin{figure}[H]
\centering
\begin{tikzpicture}
% define constants
...
% draw circle
\draw (0,0) coordinate (O) node{$\bullet$} node[left]{$O$};
\draw (0,0) circle(\radij);

\draw ($(0,0)!3cm!(4,3)$) coordinate (Interaction) node{$\bullet$};

\draw [dotted] (0,0) circle(3cm);

% draw line to X
\draw[dashed] (0,0)--(\radij + \dist,0) coordinate (X);
\draw[dashed] (0,0)-- node[below] {$R$} (\radij,0);

% draw line from P to the right
\draw[dashed] ($(0,0)!3cm!(4,3)$) -- ($(0,0)!3cm!(4,3) + (\dist,0)$) coordinate(Pright);

% draw path of scattered photon
\draw ($(0,0)!3cm!(4,3)$) -- ($(0,0)!3cm!(4,3) + (0,0)!2cm!(1,3)$) coordinate (Scattered_location);

% draw line to P and further
\draw[dashed] (0,0) -- node[above] {$r$} ($(0,0)!3cm!(4,3)$)  node[midway,above left, blue!50!black]{};

\draw[dashed] (0,0) -- ($(0,0)!5cm!(4,3)$) coordinate(Plonger) node[midway,above left, blue!50!black]{};

% define coordinates
\draw ($(\radij,0)$) coordinate (Release) node{$\bullet$};

\draw ($(0,0)!3cm!(4,3)$) coordinate (Interaction) node{$\bullet$};

  	% draw angle theta
  \draw  pic["$\theta$", draw=red,<->, angle eccentricity=1.2, angle radius=0.5cm]
    {angle=X--Release--Interaction};
	
	% draw angle theta_in
  \draw  pic["$\theta_{in}$", draw=red, ->, angle eccentricity=1.2, angle radius=1cm]
    {angle=X--O--Interaction};
    
    	% draw angle theta_out
  \draw    pic["$\theta_{out}$", draw=red, ->, angle eccentricity=1.5, angle radius=1cm]
    {angle=Plonger--Interaction--Scattered_location};
    
  \draw    pic["$\alpha$", draw=red, ->, angle eccentricity=1.2, angle radius=0.5cm]
    {angle=O--Interaction--Release};
    % draw a horizontal line
    
    %\draw[dashed] ($(0,0)!3cm!(4,3)$) -- ($(0,0)!3cm!(4,3)+ (\dist,0)$) ;

	% draw the release of the photon
	\draw[->] ($(\radij,0)$) -- ($(0,0)!3cm!(4,3)$);
	
	% draw the oberver's eye
	\eye{0.5}{5}{4}{-140}
\end{tikzpicture}
\caption{2D Geometry for scattering}
\label{1D_Geometry_scattering}
\end{figure}
...
\subfile{../small_observations/observations}
...
In the case $\mu_{\text{out}} < 0$ then
\begin{figure}
\centering
\begin{tikzpicture}
% define constants
...
\draw ($(0,0)!3cm!(3,4)$) coordinate (Interaction) node{$\bullet$};

\draw [dotted] (0,0) circle(3cm);
\draw [dotted] (0,0) circle(6cm);

\draw[dashed] (0,0) -- node[below] {$r_{\text{init}}$} (\radij + \dist,0);

\draw ($(0,0)!3cm!(3,4)$) -- ($(0,0)!6cm!(-2,4)$) node {$\bullet$} coordinate (Final);

\path (0,0)--  (\radij + 2*\dist,0) coordinate (X);

% draw line to P and further
\draw[dashed] (0,0) -- node[above] {$r_{\text{nearest}}$} ($(0,0)!3cm!(3,4)$)  node[midway,above left, blue!50!black]{};

% define coordinates
\draw ($(\radij+\dist,0)$) coordinate (Release) node{$\bullet$};

\draw ($(0,0)!3cm!(3,4)$) coordinate (Interaction) node{$\bullet$};

% draw angle theta
\draw  pic["$\pi-\theta_{\text{out}}$", draw=red,<->, angle eccentricity=2.7, angle radius=0.5cm]
    {angle=Interaction--Release--O};
    
\draw  pic["$\pi/2$", draw=red,<->, angle eccentricity=1.2, angle radius=0.5cm]
    {angle=O--Interaction--Release};
	   
% draw the release of the photon
\draw[->] ($(\radij + \dist,0)$) -- ($(0,0)!3cm!(3,4)$);
	
\end{tikzpicture}
\caption{Trick when computin $L(r)$}
\label{Computing_L}
\end{figure}
...
\noindent\fbox{
  \parbox{0.6\textwidth}{
  @ Luka, do you know where this expression comes from?}}
\begin{equation}
