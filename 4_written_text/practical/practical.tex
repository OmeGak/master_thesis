...
\item Revisit convergence analysis: adapt plot formatting and standard deviation is defined as square root of variance (see Section \underline{\ref{convergence_analysis}}). (OK)

\item Test variance reduction technique (see Section \underline{\ref{variance_reduction_experiment}})  

\item Some general considerations about the definitiion of specific intensity (see Section \underline{\ref{specific_intensity}})
...
\newpage
\subsubsection{Closer look at Monte Carlo simulations}
\label{diffusion_Monte_Carlo_mean_free_path}

\paragraph{Random walk (diffusion equation)} A more simple experiment that simulates the diffusion equation (1D random walk) is also set up. The results are shown in Figure \ref{random_walk_N_vs_tau}.
	\begin{figure}[!htp]
	\centering
...
\begin{itemize}
\item When starting from an initial condition $x_0 = 0$,
\begin{equation}
x_N = x_{N-1} \pm l
\end{equation}
the variance equals $\braket{x_N}^2 = N l^2$ 
\item If we require a photon to cover a distance $R$ then $N = \frac{R^2}{l^2}$ and
\begin{itemize}
\item with the following relation between mean-free path $l$ and opacity $\alpha$ namely $l = \frac{1}{\alpha}$
\item with $\tau = \int_0^R \alpha ds = \frac{R}{l}$
\end{itemize}
we have that $N = \tau^2$. This corresponds with the observations in Figure \ref{random_walk_N_vs_tau}.
\end{itemize}


\newpage
\paragraph{Limb darkening} We first look at results from the limb darkening program. In Figure \ref{limb_darkening_N_vs_tau}, the number of scattering events is plotted versus the opacity of the medium. 
	\begin{figure}[!htp]
	\centering
...
\paragraph{Eddington approximation} 
\begin{equation}
...
\end{equation}
with the emergent intensity $I(0,\mu)$ as solution of  Equation (\ref{rad_transfer_eq_limb_darkening}). Its solution equals 
\begin{equation}
...
\begin{figure}[!htp]
\centering
\includegraphics[width=0.65\textwidth]{../../introductory_exercises/P_Cygni_profile_UV_resonance/data/test0_convergence.png}
...
\item Check if interaction is uberhaupt possible.
\item Solve $x_{REF} = x_{CMF} - \mu v(r)$ with $\boxed{x_{CMF} =b}$ where $b$ is the frequency where no scattering has yet found place
\item Check whether the photon is scattered:
\end{enumerate}}
...
\newpage
\section{Definition of specific intensity}
...
From this we deduce that (since, in spherical geometry $d\Omega = \sin(\theta) d\theta d\phi = d\mu d\phi$ )
\begin{equation}
I_{\nu}(\mu) \mu 
	= \frac{d E_{\nu}}{d\Omega dt d\nu d\theta} 
	= \frac{dE_{\nu}}{\sin(\theta) d\theta d\phi dt d\nu d\theta} 
	= \frac{dE_{\nu}}{d\mu d\phi dt d\nu d\theta}
	= \frac{N_{\nu}(\mu) dE_{\nu,\text{photon}}}{d\mu d\phi dt d\nu d\theta}
\end{equation}
\begin{equation}
I_{\nu}(\mu) \mu d\mu = \frac{dE_{\nu}}{d\phi dt d\nu d\theta}
\end{equation}

\noindent\fbox{
  \parbox{\textwidth}{here is something wrong}}
